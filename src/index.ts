#!/usr/bin/env node
/**
 * Lightspark SDK MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their Lightspark credentials as Bearer token.
 * The token is a base64-encoded "client_id:client_secret" string.
...
app.get('/', (_req, res) => {
  ...
  // Default: JSON
  res.json({
    name: 'Lightspark SDK MCP Server',
    provider: 'AgenticLedger',
    version: '1.0.0',
    description:
      'Lightning Network payments, invoices, channels, wallets, UMA, and compliance via Lightspark SDK.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Lightspark credentials as a Bearer token. Format: base64(client_id:client_secret). No credentials are stored on this server.',
      header: 'Authorization: Bearer <base64(client_id:client_secret)>',
      howToGetKey:
        'Generate API credentials at https://app.lightspark.com/api-config',
    },
    configTemplate: {
      mcpServers: {
        'lightspark-sdk': {
    ...
    links: {
      health: '/health',
      documentation: 'https://docs.lightspark.com/lightspark-sdk',
    },
  });
});
  ...
  const configTemplate = JSON.stringify(
    {
      mcpServers: {
        'lightspark-sdk': {
          url: `${SERVER_BASE_URL}/mcp`,
          headers: {
            Authorization: 'Bearer YOUR_BASE64_CREDENTIALS',
          },
        },
      },
    },
...
<head>
  ...
  <title>Lightspark SDK MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Lightspark SDK MCP Server</h1>
    <p class="subtitle">Lightning Network payments, invoices, channels, wallets, UMA, and compliance via Lightspark SDK.</p>

    <div class="card">
      <h2>How to get your Lightspark API credentials</h2>
      <ol class="steps">
        <li>Log in to <a href="https://app.lightspark.com" target="_blank" rel="noopener">Lightspark</a>.</li>
        <li>Go to <strong>API Configuration</strong> at <a href="https://app.lightspark.com/api-config" target="_blank" rel="noopener">app.lightspark.com/api-config</a>.</li>
        <li>Create or copy your <strong>Client ID</strong> and <strong>Client Secret</strong>.</li>
        <li>Base64-encode them as <code>client_id:client_secret</code> and paste below.</li>
      </ol>
    </div>
    ...
    <div class="card">
      ...
      <input type="text" id="clientId" placeholder="Your Lightspark Client ID…" autocomplete="off" spellcheck="false" />
      <label for="clientSecret">Client Secret</label>
      <input type="text" id="clientSecret" placeholder="Your Lightspark Client Secret…" autocomplete="off" spellcheck="false" />
  ...
  <script>
    ...
    function buildConfig(id, secret) {
      const token = (id && secret) ? btoa(id + ':' + secret) : 'YOUR_BASE64_CREDENTIALS';
    ...
    function updateConfig() {
      configBlock.textContent = buildConfig(clientIdInput.value.trim(), clientSecretInput.value.trim());
    }

    clientIdInput.addEventListener('input', updateConfig);
    clientSecretInput.addEventListener('input', updateConfig);
...
app.get('/health', (_req, res) => {
  res.json({
    status: 'ok',
    server: 'lightspark-sdk-mcp-http',
    version: '1.0.0',
    tools: tools.length,
    transport: 'streamable-http',
    auth: 'dual-mode',
...
// --- Extract Bearer token and decode to client_id:client_secret ---
function extractCredentials(req: express.Request): { clientId: string; clientSecret: string } | null {
...
function createMCPServer(client: LightsparkClient): Server {
  const server = new Server(
    { name: 'lightspark-sdk-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  // New session — requires Bearer token (base64-encoded client_id:client_secret)
  const credentials = extractCredentials(req);
  if (!credentials) {
    res.status(401).json({
      error: 'Missing or invalid Authorization header. Use: Bearer <base64(client_id:client_secret)>',
    });
    return;
  }

  // Create per-session API client with the user's credentials
  const client = new LightsparkClient(credentials.clientId, credentials.clientSecret);
...
<head>
  ...
  <title>Authorize \u2014 Lightspark SDK MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Lightspark SDK MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Lightspark SDK MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Lightspark SDK MCP HTTP Server running on port ${PORT}`);
